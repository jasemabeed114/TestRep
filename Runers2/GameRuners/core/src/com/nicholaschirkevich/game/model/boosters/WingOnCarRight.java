 ...
 */
public class WingOnCarRight extends WingOnCarBasicRight {
    private Random rand;
    ...
    public WingOnCarRight(World world, int x, int y, int movement) {
        super(x, y, movement);
        defaultX = x;
        defaultY = y;
        ...
        if (isLeft)
            position.x = Constants.getCarPostitionXLeft(wingOnCarRight.getKeyFrames()[0].getRegionWidth());
        else
            position.x = Constants.getCarPostitionXRight(wingOnCarRight.getKeyFrames()[0].getRegionWidth());
        ...
        bodyDef.position.set((wingOnCarRight.getKeyFrames()[0].getRegionX() + wingOnCarRight.getKeyFrames()[0].getRegionWidth() / 2)
                ,
                (wingOnCarRight.getKeyFrames()[0].getRegionY() + wingOnCarRight.getKeyFrames()[0].getRegionHeight() / 2));

        body = world.createBody(bodyDef);

        PolygonShape shape = new PolygonShape();
        shape.setAsBox(wingOnCarRight.getKeyFrames()[0].getRegionWidth() / 2, wingOnCarRight.getKeyFrames()[0].getRegionHeight()
                / 2);
