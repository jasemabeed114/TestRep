 ...
 */
public class TutorialBlocks extends Prize {
    private Random rand;
    ...
    public TutorialBlocks(World world, int x, int y, int movement) {
        super(x, y, movement);
        defaultX = x;
        defaultY = y;
        rand = new Random();
...
//        body.createFixture(fixtureDef);
        stateTime = 0f;
        for(int i =0;i<5;i++){
          blocks.add(new Block(world,(int)Constants.getCarPostitionXLeft(springboardAnimation.getKeyFrames()[0].getRegionWidth())-15+120,(int)position.y+90+i*50,10));
        }
//        for(int i =0;i<3;i++){
...
//        }
    ...
    public TutorialBlocks(World world, int x, int y, int movement, boolean ifLeft) {
        super(x, y, movement);
        defaultX = x;
        defaultY = y;
...
//        body.setTransform(position.x, position.y, 0.0f);
    ...
    public static void updateCoins(ArrayList<TutorialBlocks> busterses, Camera camera, float dt) {
...
//            if (!isBigCar)
            busterses.add(new TutorialBlocks(world, 90, (int) camera.viewportHeight + 20, 10));
//            else
