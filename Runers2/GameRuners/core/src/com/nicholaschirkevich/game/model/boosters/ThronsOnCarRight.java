...
import com.nicholaschirkevich.game.userdata.ThornsOnCarDataType;
 ...
 */
public class ThronsOnCarRight extends ThronsOnCarBasicRight {
    private Random rand;
    ...
    public ThronsOnCarRight(World world, int x, int y, int movement) {
        super(x, y, movement);
        defaultX = x;
        defaultY = y;
        ...
        if (isLeft)
            position.x = Constants.getCarPostitionXLeft(thronsOnCarRight.getKeyFrames()[0].getRegionWidth());
        else
            position.x = Constants.getCarPostitionXRight(thronsOnCarRight.getKeyFrames()[0].getRegionWidth());
        ...
        bodyDef.position.set((thronsOnCarRight.getKeyFrames()[0].getRegionX() + thronsOnCarRight.getKeyFrames()[0].getRegionWidth() / 2)
                ,
                (thronsOnCarRight.getKeyFrames()[0].getRegionY() + thronsOnCarRight.getKeyFrames()[0].getRegionHeight() / 2));

        body = world.createBody(bodyDef);

        PolygonShape shape = new PolygonShape();
        shape.setAsBox(thronsOnCarRight.getKeyFrames()[0].getRegionWidth() / 2, thronsOnCarRight.getKeyFrames()[0].getRegionHeight()
                / 2);
      ...
      //  fixtureDef.filter.categoryBits = THORN;
       // fixtureDef.filter.maskBits = MyCar.MY_CAR_FILTER_ENTITY;
        body.setUserData(new ThornsOnCarDataType());
